...
# Utility functions for Shell plugin hooks
...
# Check if project has local hook override for shfmt
has_project_hook_override() {
    ...
    # Check if settings has PostToolUse hooks with shfmt
    if jq -e ".hooks.PostToolUse[]?.hooks[]?.command | select(test(\"shfmt\"; \"i\"))" "$project_settings" >/dev/null 2>&1; then
        return 0
    fi
